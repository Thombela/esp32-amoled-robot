...
firmware/src/
  ...
  ui.{h,cpp}                — 3-tile swipeable UI (Home, Claude, Settings) built on LVGL's lv_tileview. compute_layout() picks fonts/positions from board_caps() (responsive — current breakpoint: H >= 460 → large, else compact)
...
The boot screen is `SCREEN_HOME` and only advances on a physical swipe, so a fresh flash will sit on Home. To screenshot the tile you're actually editing without asking the user to swipe, **temporarily change the default boot screen** in `main.cpp` (search for `ui_show_screen(SCREEN_HOME);`) to `SCREEN_CLAUDE` / `SCREEN_SETTINGS`, do your iteration, then revert before committing.
...
`tools/png_to_lvgl.js <input.png> <symbol> [W_MACRO] [H_MACRO] [--tint=RRGGBB | --no-tint]` converts an alpha PNG to RGB565A8. Default tint is white (`0xFFFFFF`) — necessary for Lucide PNGs. Splice output into `firmware/src/icons.h` and use `init_icon_dsc_rgb565a8()` in ui.cpp. Only the 5 battery icons use this format today.

## Backgrounds

`tools/img_to_lvgl.py <input.jpg> <symbol> <MACRO_PREFIX> --size WxH --out <output.h>` converts a photo (any Pillow-readable format) to a flat opaque RGB565 array — no alpha plane, unlike the icon pipeline above. Source images live in `firmware/assets/backgrounds/`; output goes to `firmware/src/backgrounds.h` (generated, do not hand-edit). Only one background is wired up today, hardcoded as the Home tile's image in `ui.cpp`'s `init_home_tile()` — a picker for multiple backgrounds doesn't exist yet.
