...
a new folder + a new `[env:...]` block — `main.cpp` and `ui.cpp`
...
- `boards/waveshare_amoled_206/` — Waveshare ESP32-S3-Touch-AMOLED-2.06 (CO5300, 410×502 watch form factor, FT3168 touch, no IO expander, 32 MB flash, PCF85063 RTC, ES8311 codec). Build env: `waveshare_amoled_206`. Display, touch, battery, IMU init, and BLE verified on hardware; `sound.cpp` no-ops (the chime engine was removed from all boards, see below).

**C6 ports have no PSRAM** — shared code gates on `BOARD_HAS_PSRAM` (absent on C6) to use `MALLOC_CAP_INTERNAL` for LVGL buffers, and the `screenshot` serial command is disabled (`LV_USE_SNAPSHOT=0`), so UI changes on a C6 board must be eyeballed on hardware, not auto-captured.
...
- Buttons: GPIO 0 (left → Space/voice-mode), GPIO 18 (right → Shift+Tab/mode-toggle), AXP PKEY (middle → cycle brightness; hold 3s + release → pairing mode)
...
- Buttons: GPIO 0 (BOOT → Space/voice-mode), XCA9554 EXIO4 (PWR → cycle brightness; hold 3s + release → pairing mode). **No third button** (GPIO 18 button doesn't exist on this board).
...
- Buttons: **GPIO 9** (BOOT → Space/voice-mode, active LOW — *not* the docs' GPIO 0/9 guess; confirmed by scan), **AXP2101 PKEY** (PWR → cycle brightness; hold 3s + release → pairing mode). The PKEY **SHORT-press IRQ fires on release** — that's the edge `power.cpp` acts on. No secondary button.
...
- Audio codec: **ES8311** + ES7210 ADC on the same I2C bus. `sound.cpp` no-ops — the chime engine was removed from all boards to save flash (see below).
...
The boot screen is `SCREEN_HOME` and only advances on a physical swipe, so a fresh flash will sit on Home. To screenshot the tile you're actually editing without asking the user to swipe, **temporarily change the default boot screen** in `main.cpp` (search for `ui_show_screen(SCREEN_HOME);`) to `SCREEN_LIBRARY` (the category-card grid) or `SCREEN_CLAUDE` / `SCREEN_SETTINGS` (jumps straight to that app's full-screen overlay, bypassing the Library popup — `ui_show_screen()` still supports this even though those two aren't tileview tiles anymore), do your iteration, then revert before committing.
...
8. **LVGL RGB565A8 is planar.** `w*h` RGB565 pixels followed by `w*h` alpha bytes; `data_size = w*h*3`, `stride = w*2`. Use `init_icon_dsc_rgb565a8()` for icons that overlap non-uniform backgrounds (e.g. the always-on-top battery indicator over any tile). Lucide source PNGs are black-on-transparent — converter must tint to white or icons render invisible. See `tools/png_to_lvgl.js`. A full-screen opaque photo (Home's background) is different — plain RGB565, no alpha plane — see `init_bg_dsc_rgb565()` in `ui.cpp` and `tools/img_to_lvgl.py`.
...
Source images live in `assets/` at the repo root — icons/logos in
`assets/apps/`, wallpapers in `assets/background/` — not under
`firmware/assets/` (that path is gone; everything was consolidated into the
root `assets/` tree). Generated `firmware/src/*.h` headers are not hand-edited.
...
- **Library screen navigation/gesture polish + 6-category test scaffold (2026-07-31).** Following up on the Library screen below: (1) Settings and the Library grid are now vertically scrollable (`LV_DIR_VER`, `make_panel()` gained `LV_OBJ_FLAG_GESTURE_BUBBLE` so a swipe starting on a settings-row panel or category card still reaches the handler above it) — a real scrollbar/bounce-at-the-end makes "is there more content" obvious instead of a dead screen. (2) The "< Library" text back-button is gone; each app overlay now has a `back_gesture_cb` (swipe left anywhere) plus a centered, 50%-wide, iOS-style home-indicator bar (`make_home_indicator()`, tap or swipe up) at the bottom — content sub-objects (`claude_content`/`settings_content`/etc.) are sized to `scr_h - zone_h` so the indicator's touch target never fights the screen's own vertical scroll. (3) **Touch-rotation fix on the 2.16 (`boards/waveshare_amoled_216/touch.cpp`):** `correct_touch()` compensates raw touch coordinates for the IMU-driven display rotation (`imu_hal_rotation_quadrant()`) so a tap/swipe lands on the LVGL object actually drawn under the finger regardless of physical orientation. **This took three attempts to get right — the key lesson, if this ever needs revisiting, is that `imu_hal_rotation_quadrant()`'s index does NOT map to physical orientation the way you'd guess** (the orientation with buttons on top, the seemingly-obvious "rotation 0", is actually **rotation 3**; buttons-on-right is rotation 2; buttons-on-bottom/upside-down is rotation 1; buttons-on-left is rotation 0). Two earlier attempts were each calibrated against data that, in hindsight, either mislabeled which physical orientation was being tested or wasn't independently re-verified against a fresh hardware log before shipping — both produced formulas that looked algebraically plausible but were wrong on real hardware, once even regressing orientations that had previously tested correctly. The fix that actually held up: log raw (pre-correction) touch coordinates alongside `imu_hal_rotation_quadrant()`'s value, tap all 4 corners in a known order plus two cardinal swipes, at *each* of the 4 physical orientations in one sitting (a `Brightness cycled` marker — mashing the PWR button a few times between orientations — is an easy way to find section breaks in the serial log afterward). The 4 resulting per-rotation formulas in `correct_touch()` form a proper rotation group (rotation 0 and 2 are exact inverses of each other; either applied twice gives rotation 1's 180° formula) — that internal self-consistency, on top of matching the hardware log, is what finally gave real confidence. Don't hand-derive a replacement from partial data or symbolic reasoning alone — re-run the full 4-orientation log first. (4) Added 4 more Library categories — Social (WhatsApp), Entertainment (Apple Music), Fitness (Strava), Photo (Photos) — as a **testing scaffold** (real logos, `SCREEN_PLACEHOLDER` instead of a real screen) to exercise the now-2-column/3-row scrollable grid; remove once real apps land there or they're no longer needed. (5) App icons switched from opaque JPEG (clip-masked into squircles at draw time) to alpha PNG (`png_to_lvgl.js --no-tint`, RGB565A8) — the source logos already carry their own rounded-square shape + transparent margins, so `make_app_icon()` no longer needs a wrapper/clip-mask object at all.
- **Library screen replaces the Claude/Settings tiles (2026-07-27).** The 3-tile swipe (Home/Claude/Settings) is now 2 tiles: Home and an iOS App Library-style **Library** tile (`init_library_tile()` in `ui.cpp`) — a blurred-wallpaper background (`bg_library_data`) behind category folder cards, built off small `CATEGORIES[]`/`APPS[]` tables so a third app is one data row, not new screen code (now 6 categories/2 columns — see the entry above). Tapping a card opens a floating popup (`open_category_popup()`) listing that category's apps (kept alphabetically ordered in `APPS[]` — no runtime sort, not worth it at 1-2 entries/category); tapping a row opens the app's real screen, a full-screen overlay (`overlay_claude`/`overlay_settings`/`overlay_placeholder`) instead of a tileview tile (there's no free hardware button for back — both buttons on every board are already claimed for BLE HID actions, so navigation is fully touch-driven, same as iOS's own App Library — see the gesture/home-indicator entry above for how "back" actually works now). Tapping the popup's dimmed scrim (outside the card) dismisses it — LVGL objects don't bubble click events by default, so the card absorbs taps on its own empty area for free.
- **Chime engine removed to save flash (2026-07-27).** Deleted the shared ES8311 chime engine (`chime.{h,cpp}`), its vendored codec driver (`es8311.{c,h}`, `es8311_reg.h`), and the embedded reset-clip PCM (`bell_pcm.h`, ~1 MB of `.rodata` baked into every board's binary regardless of whether that board actually played it). Every board's `sound.cpp` — including the 2.16, 1.8, and 1.54, which used to drive the real codec — is now a no-op, same posture the unwired boards (2.06, both C6 ports) already had. `sound_hal.h`'s contract, the `buzz` serial command, and the daemon's opt-in `usage.chime` flag are unchanged; they just have nothing to play now. Re-add a real chime engine later if flash headroom allows.
...
- **WiFi usage-data path + 3-tile paging UI (2026-07-27).** Added a WiFi HTTP poller (`wifi_poll.{h,cpp}`) decoupled from BLE ownership — needed since the board can be BLE-bonded to a phone (HID duties) while a Windows daemon's local HTTP endpoint (tunneled via ngrok) still delivers usage data. Replaced the pixel-art splash screen + detailed Usage screen entirely with a 3-tile swipeable UI (Home background photo / Claude minimal %/Settings live status) built on `lv_tileview`. Removed the now-dead splash subsystem, `logo.h`, two unused icon arrays, and their now-orphaned generator tooling.

## Daemon / host side

Bash daemon (`daemon/claude-usage-daemon.sh`, macOS/Linux) reads OAuth token, polls Anthropic API, sends JSON over BLE GATT. Run with `systemctl --user start claude-usage-daemon`. The unit file's `ExecStart` is the absolute path to the script — repoint it when switching between the worktree and the main checkout.

**Windows** (`daemon/claude_usage_daemon_windows.py`) is the actively-used daemon for this project's current hardware. It runs the same BLE delivery loop (`_ble_loop`) alongside an independent `usage_poll_loop` that polls Anthropic on its own clock regardless of BLE connection state — this is what lets a local HTTP endpoint (`daemon/http_server_windows.py`, `GET /usage`, bound to `127.0.0.1` only, `X-Clawd-Key` header auth) keep serving fresh data even when the board is BLE-bonded to something else (e.g. a phone) and can no longer receive BLE writes from this PC. Enable via `http_enabled`/`http_port`/`http_secret` in the config file (see `daemon/config.example`); expose it to the internet with a tunnel (ngrok free static domain works well) for the firmware's `wifi_poll.cpp` to reach.
