...
//           panel is dark, so pets/sleeves can't wake it overnight or swipe
//           between tiles on a black panel.
...
// 160, not 64: a WPA2 password or a full https://....ngrok-free.app/usage URL
// (wifi_pass / wifi_url commands) won't fit in 64 bytes.
...
// `key value` prefix match — takes the rest of the line after `key ` as the
// setter's argument, verbatim. Avoids delimiter/escaping problems from packing
// multiple fields (ssid/pass/url) into one command.
static void dispatch_kv(const char* buf, const char* key, void (*setter)(const char*)) {
    size_t klen = strlen(key);
    if (strncmp(buf, key, klen) == 0 && buf[klen] == ' ') setter(buf + klen + 1);
}
...
static void check_serial_cmd() {
    while (Serial.available()) {
        ...
        if (c == '\n' || c == '\r') {
            ...
            else if (strcmp(cmd_buf, "wifi_status") == 0) wifi_poll_print_status();
            else {
                dispatch_kv(cmd_buf, "wifi_ssid", wifi_poll_set_ssid);
                dispatch_kv(cmd_buf, "wifi_pass", wifi_poll_set_pass);
                dispatch_kv(cmd_buf, "wifi_url", wifi_poll_set_url);
                dispatch_kv(cmd_buf, "wifi_secret", wifi_poll_set_secret);
            }
            cmd_pos = 0;
...
void loop() {
    ...
    // so no separate idle_note_activity() call is needed here.
    {
        ...
        if (power_hal_pwr_pressed()) {
            if (!idle_consume_wake_press()) {
                brightness_cycle();
                ui_refresh_settings();  // keep Settings' brightness row live if visible
            }
        }
    ...
    if (ble_has_data()) {
        if (usage_apply_json(ble_get_data())) ble_send_ack();
        else                                  ble_send_nack();
    }
