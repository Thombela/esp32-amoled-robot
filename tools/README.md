...
`assets/background/`); generated output goes to `firmware/src/ui/icons/*.h`
and `firmware/src/ui/backgrounds/*.h` (do not hand-edit those, they're
regenerated from `assets/`).
...
spliced into an existing header (`ui/icons/icons.h`). When building a new
standalone header out of several of its outputs (e.g. `app_icons.h`), redirect the first
...
Output goes to `firmware/src/ui/backgrounds/backgrounds.h`. Wrap with
